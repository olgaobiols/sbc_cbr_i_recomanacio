...
from operadors_tecniques import (
...
def _print_banner(title: str) -> None:
    """Imprimeix capçalera principal de l'aplicació."""
    line = _line("=")
    print(line)
    print(title)
    print(line)
...
def _print_block(title: str, ch: str = "-") -> None:
    """Imprimeix un bloc destacat amb separadors."""
    line = _line(ch)
    print("\n" + line)
    print(title)
    print(line)
...
def _print_section_line(title: str, ch: str = "=") -> None:
    """Imprimeix una línia de secció amb títol compacte."""
    prefix = f"=== {title} "
...
def input_optional(prompt: str, placeholder: str = "Enter") -> str:
...
def input_choice(
    ...
    options: List[str],
    default: str,
    indiferent_value: Optional[str] = None,
    show_options_in_prompt: bool = True,
) -> str:
    """Demana una opció d'una llista, amb suport per 'indiferent'."""
    opts = list(options)
...
def input_int_default(prompt: str, default: int) -> int:
    txt = input(f"{prompt} [{default}]: ").strip()
    if not txt:
        return default
    try:
...
def input_float_default(prompt: str, default: float, display_default: Optional[str] = None) -> float:
    shown = display_default if display_default is not None else str(default)
...
def _collect_allergen_restrictions(items: List[str]) -> Set[str]:
    """Extreu restriccions que corresponen a al·lèrgens UE."""
    if not items:
        return set()
...
def _normalize_item(value: str) -> str:
    """Normalitza tokens (lowercase + espais) per a comparacions robustes."""
    if not value:
        return ""
...
def _display_dieta_tag(value: str) -> str:
    """Mostra dietes amb etiqueta curta quan toca."""
    if not value:
        return value
...
def _normalize_dieta_tag(value: str) -> str:
    """Unifica variants de dietes a un vocabulari canònic."""
    if not value:
        return ""
...
def _infer_dieta_from_restriccions(restriccions_set: Set[str]) -> Optional[str]:
    """Extreu una dieta canònica a partir d'un conjunt de restriccions."""
...
def _format_techniques(transf: Optional[List[Any]], kb_ref: Optional[Any] = None) -> str:
    """Serialitza tècniques/transformacions en una línia llegible."""
    if not transf:
        return "cap"
    ...
    for t in transf:
        if isinstance(t, str):
            ...
            if obj:
                if kb_ref:
                    obj = ingredient_ca(kb_ref, obj) or obj
                name = f"{name} de {obj}" if name else obj
        else:
            name = str(t).strip()
    ...
    return ", ".join(resultat) if resultat else "cap"


def _parse_pairs_input(txt: str) -> List[str]:
    """Parseja parelles en format 'A+B, C|D' i retorna claus 'a|b' normalitzades."""
    if not txt:
        return []
    ...
    for part in txt.split(","):
        ...
        if a_norm and b_norm:
            out.append("|".join(sorted([a_norm, b_norm])))

    return out


def _normalize_pair_key(raw: str) -> str:
    """Normalitza 'A+B' o 'A|B' a una clau canònica 'a|b'."""
    if not raw:
        return ""
...
def _collect_vetats(perfil: Dict[str, Any], learned_rules: Dict[str, Any]) -> Tuple[Set[str], Set[str]]:
    """Agrega vetos d'usuari i regles globals (ingredients + parelles)."""
...
def _plat_te_ingredient_vetat(ingredients: List[str], vetats: Set[str]) -> bool:
    """Retorna True si el plat conté algun ingredient vetat."""
    if not vetats:
        return False
    return any(_normalize_item(ing) in vetats for ing in ingredients)


def _plat_te_parella_vetada(ingredients: List[str], parelles_vetades: Set[str]) -> bool:
    """Retorna True si el plat conté alguna parella vetada."""
...
def _parelles_detectades(ingredients: List[str], parelles_vetades: Set[str]) -> List[str]:
    """Llista parelles vetades detectades dins del plat (claus 'a|b')."""
    if not parelles_vetades:
        return []
...
def _trobar_plat_alternatiu(
    ...
    """Busca un plat alternatiu del mateix curs que no violi vetos."""
...
def _check_compatibilitat_local(ingredient_info: Dict[str, Any], perfil_usuari: Optional[Dict[str, Any]]) -> bool:
    """Valida al·lèrgies/dieta contra metadades locals d'un ingredient."""
    if not ingredient_info:
        return False
    if not perfil_usuari:
        return True

    alergies = {_normalize_item(a) for a in (perfil_usuari.get("alergies", []) or []) if a}
    if alergies:
        alergens_ing = {
            _normalize_item(p)
            for p in str(ingredient_info.get("allergens", "") or "").split("|")
            if p
        }
        familia_ing = _normalize_item(str(ingredient_info.get("family", "") or ""))
        if alergies.intersection(alergens_ing) or (familia_ing and familia_ing in alergies):
            return False

    dieta = _normalize_item(str(perfil_usuari.get("dieta", "") or ""))
    if dieta:
        dietes_ing = {
            _normalize_item(p)
            for p in str(ingredient_info.get("allowed_diets", "") or "").split("|")
            if p
        }
        if dieta not in dietes_ing:
            return False
...
def _try_add_preferred_touch(
    ...
    preferits: List[str],
    perfil_usuari: Optional[Dict[str, Any]],
    vetats: Set[str],
    parelles_vetades: Set[str],
) -> None:
    """Prova d'afegir una preferència com a toc si encaixa amb el plat."""
    if not preferits:
        return
    ...
    for pref in preferits:
        ...
        pref_name = (info.get("ingredient_name") or pref_norm).strip() if isinstance(info, dict) else pref_norm
        for plat in plats:
    ...
    if best:
        plat_sel, ing_sel, score_sel = best
        plat_sel.setdefault("ingredients", []).append(ing_sel)

        logs = list(plat_sel.get("log_transformacio", []) or [])
        logs.append(f"Preferència: Afegit {ing_sel} com a toc (afinitat {score_sel:.2f})")
        plat_sel["log_transformacio"] = logs


def _print_section(title: str) -> None:
    """Imprimeix una secció simple amb separador."""
    line = _line("-")
    print("\n" + line)
    print(title)
    print(line)
...
def imprimir_allergens_taula() -> None:
    """Mostra la taula d'al·lèrgens UE amb 4 columnes."""
    line = _line("-")
    print("\n" + line)
...
def seleccionar_alergens() -> List[str]:
    """Demana els al·lèrgens a evitar via taula i retorna claus normalitzades."""
    ...
    return _dedup_preserve_order(seleccionats)


PATH_USER_PROFILES = os.path.join("data", "user_profiles.json")
PATH_LEARNED_RULES = os.path.join("data", "learned_rules.json")


def _load_user_profiles(path: str) -> Dict[str, Any]:
    """Carrega perfils d'usuari (JSON dict)."""
...
def _load_learned_rules(path: str) -> Dict[str, Any]:
    """Carrega regles apreses (JSON dict)."""
    if not os.path.exists(path):
        return {}
...
def _save_user_profiles(path: str, data: Dict[str, Any]) -> None:
    """Escriu perfils d'usuari de forma atòmica (tmp + replace)."""
...
def _dedup_preserve_order(items: List[str]) -> List[str]:
    """Elimina duplicats preservant l'ordre."""
    vistos: Set[str] = set()
    resultat: List[str] = []
    for x in items:
        if x in vistos:
...
def _vector_mitja(ingredients: List[str]) -> Optional[np.ndarray]:
    """Vector mitjà d'un conjunt d'ingredients (si n'hi ha cap amb embedding)."""
...
def _similitud_plat_estil(ingredients: List[str], estils_latents: Dict[str, Any], nom_estil: str) -> float:
    """Similitud cosinus entre el plat (mitjana) i el vector de l'estil latent."""
    estil_data = estils_latents.get(nom_estil, {}) or {}
    ings_estil = estil_data.get("ingredients", []) or []
...
def _safe_ascii_prompt(text: str) -> str:
    """Substitueix caràcters típics problemàtics per ASCII segur."""
    if not text:
        return text
...
def imprimir_resum_adaptacio(
    ...
    """Mostra un resum compacte de l'adaptació i retorna (afegits, resum)."""
    ingredients_despres = list(plat.get("ingredients", []) or [])
    ...
    duplicat_proposat: Optional[str] = None
    ...
    if trets and afegits:
        ...
        canvi_text = "INSERCIO"
        motiu = (
            "petit toc simbòlic per reforçar l'estil"
            if "fallback simbòlic" in log_text
            else "afegit un toc que harmonitza amb l'estil"
        )
    ...
    if condiment:
        resum = f"condiment: {condiment}" if resum == "cap canvi" else f"{resum} | condiment: {condiment}"
...
def imprimir_resum_plat_net(
    etiqueta_plat: str,
    plat: Dict[str, Any],
    transf: List[Dict[str, Any]],
    estil_cultural: Optional[str],
    estil_alta: Optional[str],
) -> None:
    """Mostra el plat i les tècniques aplicades, etiquetant origen cultural/alta."""
    nom_plat = plat.get("nom", "—")
    ...
    tecs_culturals: Set[str] = set()
    if estil_cultural:
        row_c = kb.estils.get(estil_cultural, {}) or {}
        tecs_culturals = {
            t.strip()
            for t in str(row_c.get("tecnniques_clau", "") or "").split("|")
            if t.strip()
        }
    ...
    for t in transf:
        ...
        if estil_cultural and not estil_alta:
            ...
            origen = "cultural" if nom in tecs_culturals else "alta"

        print(f"  - ({origen}) {disp} → {obj}")


def imprimir_menu_final_net(
    plat1: Dict[str, Any], transf_1: List[Dict[str, Any]],
    plat2: Dict[str, Any], transf_2: List[Dict[str, Any]],
    postres: Dict[str, Any], transf_post: List[Dict[str, Any]],
    estil_cultural: Optional[str],
    estil_alta: Optional[str],
) -> None:
    """Imprimeix el menú final amb ingredients i tècniques."""
...
def imprimir_casos(candidats: List[Dict[str, Any]], top_k: int = 5) -> None:
    """Mostra els resultats del Retriever de forma ordenada."""
    if not candidats:
        print("\nNo he trobat cap cas prou similar.")
        return

    n_show = min(top_k, len(candidats))
    print(f"\nHe trobat {len(candidats)} opcions. Et mostro les {n_show} millors:")

    def _nom_plat(plats: List[Dict[str, Any]], curs: str) -> str:
        curs_norm = str(curs).lower()
        for p in plats:
            if str(p.get("curs", "")).lower() == curs_norm:
                return p.get("nom", "—")
    ...
    def _ordre_plat(p: Dict[str, Any]) -> int:
        ordre = {"primer": 0, "segon": 1, "postres": 2}
        curs = str(p.get("curs", "")).lower()
        return ordre.get(curs, 99)

    for i, c in enumerate(candidats[:n_show], start=1):
        cas = c.get("cas", {}) or {}
        score = float(c.get("score_final", 0.0) or 0.0)
        ...
        restr = list(pr.get("restriccions", []) or [])
        str_restr = f" (Restr: {', '.join(restr)})" if restr else ""

        plats = list(sol.get("plats", []) or [])
        p1 = _nom_plat(plats, "primer")
        p2 = _nom_plat(plats, "segon")
        p3 = _nom_plat(plats, "postres")
        ...
        for p in sorted(plats, key=_ordre_plat):
            curs = p.get("curs", "?")
            nom = p.get("nom", "—")
...
def imprimir_menu_final(
    kb: Any,
    plat1: Dict[str, Any], transf_1: Optional[List[Any]], info_llm_1: Optional[Dict[str, Any]], beguda1: Optional[Dict[str, Any]], score1: float,
    plat2: Dict[str, Any], transf_2: Optional[List[Any]], info_llm_2: Optional[Dict[str, Any]], beguda2: Optional[Dict[str, Any]], score2: float,
    postres: Dict[str, Any], transf_post: Optional[List[Any]], info_llm_post: Optional[Dict[str, Any]], beguda_postres: Optional[Dict[str, Any]], score_postres: float,
    mostrar_logs: bool = True,
) -> None:
    """Imprimeix la proposta final de menú (plats, tècniques, logs i maridatge)."""
    print("\n" + _line("="))
    ...
    def _ingredients_ca(plat: Dict[str, Any]) -> str:
        ings_en = list(plat.get("ingredients", []) or [])
        ings_ca = ingredients_ca_llista(kb, ings_en) if kb else ings_en
        ings_ca = [x for x in (ings_ca or []) if x]
        return ", ".join(ings_ca) if ings_ca else "—"

    def _bloc(titol: str, text: str, indent: str = "   ") -> None:
        txt = _cap(text)
        if not txt:
            return
    ...
    def _neteja_log(log: str) -> str:
        """Neteja logs: treu fallback i intenta catalanitzar ingredient (casos típics)."""
        txt = _cap(log)
        if not txt:
            return ""
    ...
    def _neteja_condiment(cond: Any) -> str:
        """Intenta catalanitzar tokens tipus ingredient dins del condiment."""
        c = _cap(cond)
        if not c or not kb:
            return c
    ...
    plats_iter = [
        ("PRIMER PLAT", plat1, transf_1, info_llm_1, beguda1, float(score1 or 0.0)),
        ("SEGON PLAT",  plat2, transf_2, info_llm_2, beguda2, float(score2 or 0.0)),
        ("POSTRES",     postres, transf_post, info_llm_post, beguda_postres, float(score_postres or 0.0)),
    ]
    ...
    for etiqueta, plat, transf, info_llm, beguda, score in plats_iter:
        meta = info_llm or {}
        nom = _cap(meta.get("nom_nou")) or _cap(plat.get("nom")) or "—"
        desc = _cap(meta.get("descripcio_carta"))
        pres = _cap(meta.get("presentacio"))
        notes = _cap(meta.get("notes_tecnniques"))
        beguda_llm = _cap(meta.get("beguda_llm"))

        condiment = _neteja_condiment(plat.get("condiment", ""))
...
def imprimir_justificacio_begudes(
    plat1: Dict[str, Any], beguda1: Dict[str, Any], detail1: Dict[str, Any],
    plat2: Dict[str, Any], beguda2: Dict[str, Any], detail2: Dict[str, Any],
    postres: Dict[str, Any], beguda_postres: Dict[str, Any], detail_postres: Dict[str, Any],
) -> None:
    """Imprimeix una justificació textual del maridatge per a cada plat."""
    ...
    def _frase_ingredient(ingredient: Dict[str, Any], principal: bool = False) -> str:
        """Construeix una frase compacta per a un ingredient (principal o secundari)."""
        if not ingredient:
            return ""

        nom = _cap(ingredient.get("nom"))
        parts: List[str] = []

        if ingredient.get("categoria_macro"):
            parts.append(f"amb categories com {_cap(ingredient.get('categoria_macro'))}")
        if ingredient.get("familia"):
            parts.append(f"amb la família {_cap(ingredient.get('familia'))}")
        if ingredient.get("sabors_match"):
            parts.append(f"amb sabors com {_join(ingredient.get('sabors_match'))}")
        if ingredient.get("sabors_conflicte"):
            parts.append(
                f"tot i que no combina amb sabors com {_join(ingredient.get('sabors_conflicte'))}"
            )
        ...
        if principal:
            suf = (
                f" pròpies de l'ingredient principal {nom}, que té més pes que la resta."
                if not (ingredient.get("sabors_match") or ingredient.get("sabors_conflicte"))
                else f" pròpis de l'ingredient principal {nom}, que té més pes que la resta."
            )
            return base + suf

        return f"{nom}: {base}" if nom else base

    def _text_beguda(plat: Dict[str, Any], beguda: Dict[str, Any], detail: Dict[str, Any]) -> str:
        """Construeix el text final de justificació d'un plat."""
        plat_nom = _cap(plat.get("nom")) or "—"
        beguda_nom = _cap((beguda or {}).get("nom")) or "—"
        txt = f"\nPer al plat: {plat_nom}, s'ha escollit la beguda: {beguda_nom}."

        frases: List[str] = []
        detail = detail or {}

        ing_principal = detail.get("ingredient_principal") or {}
        if ing_principal:
            frase = _frase_ingredient(ing_principal, principal=True)
            if frase:
                frases.append("\nJa que aquesta marida " + frase)

        secundaris = list(detail.get("ingredients_secundaris", []) or [])
        if secundaris:
            secundaris_texts = [t for t in (_frase_ingredient(ing) for ing in secundaris) if t]
            if secundaris_texts:
                if len(secundaris_texts) == 1:
                    frases.append("\nTambé es té en compte l'ingredient secundari " + secundaris_texts[0])
                else:
                    frases.append("\nTambé es tenen en compte els ingredients secundaris " + "; ".join(secundaris_texts))

        extra = " ".join(frases).strip()
        return (txt + (" " + extra if extra else "") + ".").strip()

    print(_text_beguda(plat1, beguda1, detail1))
    print("\n" + ("-" * UI_WIDTH) + "\n")
    print(_text_beguda(plat2, beguda2, detail2))
    print("\n" + ("-" * UI_WIDTH) + "\n")
    print(_text_beguda(postres, beguda_postres, detail_postres))
...
def main():
    ...
    def _diff_ingredients(base: List[str], variant: List[str]) -> List[str]:
        base_list = [ing for ing in (base or []) if ing]
        var_list = [ing for ing in (variant or []) if ing]
        base_counts = Counter(base_list)
        var_counts = Counter(var_list)

        canvis = []
        for ing, count in (base_counts - var_counts).items():
            for _ in range(count):
                canvis.append(f"{ing} -> (eliminat)")
        for ing, count in (var_counts - base_counts).items():
            for _ in range(count):
                canvis.append(f"(afegit) {ing}")
        return canvis
    ...
    while True:
        ...
        if estil_latent:
            ...
            if suggerits:
                ...
                txt = input_default("En vols triar un? Escriu el número ", "0").strip()
                try:
                    idxc = int(txt)
                except ValueError:
                    idxc = 0
        ...
        if subgroups:
            ...
            _, parelles_vetades_global = _collect_vetats({}, learned_rules)
            _, parelles_vetades_vip = _collect_vetats(perfil_guardat, learned_rules) if perfil_guardat else (set(), set())
            ...
            for grup in subgroups:
                ...
                parelles_grup = parelles_vetades_vip if grup.get("is_vip") else parelles_vetades_global
                ...
                for idx_plat, (plat_base, plat_variant) in enumerate(
                    ...
                    if parelles_grup:
                        parelles_detectades = _parelles_detectades(ingredients, parelles_grup)
                        if parelles_detectades:
                            a, b = parelles_detectades[0].split("|", 1)
                            norm_map = {_normalize_item(i): i for i in ingredients if i}
                            ing_forcat = norm_map.get(b) or norm_map.get(a)
                            if ing_forcat:
                                prohibits.add(ing_forcat)
                    if prohibits:
                        adaptat = substituir_ingredients_prohibits(
                            plat_variant,
                            ...
                            parelles_prohibides=parelles_grup,
                        )
                        if isinstance(adaptat, dict):
                ...
                if not canvis and violacions_pendents:
                    ...
                    for canvi in canvis:
                        ...
                        for diff in diffs:
                            if "-> (eliminat)" in diff:
                                eliminats.append(diff.split("->", 1)[0].strip())
                            elif diff.startswith("(afegit)"):
                                afegits.append(diff.replace("(afegit)", "").strip())
                            elif "->" in diff:
                                canvis_directes.append(diff)

                        parts = []
                        if canvis_directes:
                            parts.append(f"Canvis: {', '.join(canvis_directes)}")
                        if eliminats:
                            parts.append(f"Eliminats: {', '.join(eliminats)}")
                        if afegits:
                            parts.append(f"Afegits: {', '.join(afegits)}")
                        canvi_txt = " | ".join(parts) if parts else "canvis d'ingredients"
                        print(f"   - {canvi['curs']}: {canvi_txt}")

                    nom_p1 = plats_variant[0].get("nom", "—")
                    nom_p2 = plats_variant[1].get("nom", "—")
                    nom_p3 = plats_variant[2].get("nom", "—")
                    print(f"   Resum menú adaptat: Primer {nom_p1} | Segon {nom_p2} | Postres {nom_p3}")
