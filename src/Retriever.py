...
------------------------------
Implementa l'estratègia Adaptation-Guided Retrieval (Secció 2.1 PDF).
Prioritza casos no només per similitud superficial sinó per viabilitat d'adaptació
(especialment restriccions alimentàries).
"""

class Retriever:
    # Pesos d'Agregació Global (Secció 2.1.3)
    # Prioritzem estructura (Event/Servei) i seguretat (Restriccions)
    W = {
        'event': 0.30, 'servei': 0.25, 'restr': 0.20,
        'temp': 0.10, 'formal': 0.10, 'pax': 0.03, 'preu': 0.02
    }
    
    # Vocabularis i Grups Semàntics (Matrius de Similitud)
    ...
    # MÈTRIQUES DE SIMILITUD LOCAL (Secció 2.1.2)
    ...
    def _sim_restriccions(self, req_user: List[str], case_restr: List[str]) -> float:
        """Jaccard Index: (Intersecció / Unió). Penalitza inseguretat."""
        u = {self._norm(r) for r in req_user if r}
        c = {self._norm(r) for r in case_restr if r}
        if not u: return 1.0 # Si l'usuari no té restriccions, tot val
        unio = len(u | c)
    ...
    def _sim_temporada(self, a: str, b: str) -> float:
        ...
        if a == b or "indiferent" in (a, b): return 1.0
        try:
            dist = abs(self.SEASONS.index(a) - self.SEASONS.index(b))
            return 1.0 - (min(dist, 4 - dist) * 0.5)
        except ValueError: return 0.4

    def _sim_preu(self, target, actual) -> float:
    ...
    def _sim_pax(self, target, actual) -> float:
        """Diferència relativa inversa (Logística)."""
        try:
    ...
    def _score(self, req: DescripcioProblema, cas: dict) -> dict:
        p = cas.get("problema", {})
        ...
        # Agregació ponderada
        score = sum(self.W[k] * v for k, v in sims.items())
        return {"score_final": score, "detall": sims}

    def recuperar_casos_similars(self, peticio: DescripcioProblema, k: int = 3):
        """Retorna els top-k casos (k-NN) ordenats per score."""
        scored = []
        for cas in self.base_casos:
            s = self._score(peticio, cas)
            scored.append({**s, "cas": cas})
        
        return sorted(scored, key=lambda x: x["score_final"], reverse=True)[:k]